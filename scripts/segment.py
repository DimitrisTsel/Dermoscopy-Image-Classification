...
def segment_lesion(image,mode="None"):
    ## Unsupervised Segmentation
    # K-Means Clustering
    if (mode=="KMeans"):
        # K-Means Clustering
        deim    = denoise(cv2.cvtColor(image, cv2.COLOR_RGB2GRAY), weight=500)
        kmeans  = KMeans(n_clusters=2, random_state=0).fit(deim.reshape(-1,1))
        mask    = (kmeans.labels_).reshape(deim.shape[0],deim.shape[1]).astype('uint8')
    
    # Expectation-Maximization Gaussian Mixture Model    
    elif (mode=="EM"):
        # K-Means Clustering
        feature_vector             = (denoise(cv2.cvtColor(image, cv2.COLOR_RGB2GRAY), weight=100)).reshape(-1,1)
        kmeans                     = KMeans(n_clusters=2, random_state=0).fit(feature_vector)
    ...
    # Chan-Vese Active Contours   
        ...
        evolution = []
        callback = store_evolution_in(evolution)
        mask = morphological_geodesic_active_contour(gimage, num_iter=230,
                                           init_level_set=init_ls,
                                           smoothing=1, balloon=-1,
                                           threshold=0.69,
                                           iter_callback=callback)
        ...
        evolution = []
        callback  = store_evolution_in(evolution)
        mask      = morphological_chan_vese(image, num_iter=35, init_level_set=init_ls, smoothing=200, iter_callback=callback).astype(np.uint8)
        '''
    # Watershed   
    elif (mode=="mod_watershed"):

       ret, thresh = cv2.threshold(cv2.cvtColor(image, cv2.COLOR_RGB2GRAY),0,255,cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
       ...
       circle_mask = create_circular_mask(image.shape[0], image.shape[1], radius = 220)
       ls          = ls * circle_mask
    ...
    '''  
       ...
       # Post-Process Masks
       post = ndimage.binary_fill_holes(large, structure=np.ones((5,5))).astype(bool)
    '''
